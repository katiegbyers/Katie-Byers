...
public class KDTreeTest {
    ...
    private void testWithPointsAndQueries(int pointCount, int queryCount) {
        ...
        NaivePointSet numPoints = new NaivePointSet(points);
        KDTree kd = new KDTree(points);

        List<Point> queries = randomPoints(queryCount);
        for (Point p : queries) {
            Point expected = numPoints.nearest(p.getX(), p.getY());
            Point actual = kd.nearest(p.getX(), p.getY());
            assertEquals(expected, actual);
        }
    }

    private void timeWithPointsAndQueries(int pointCount, int queryCount) {
        List<Point> points = randomPoints(pointCount);
        KDTree kd = new KDTree(points);
        ...
        System.out.println("Time elapsed for " + queryCount + " queries on "
                + pointCount + " points: " + time.elapsedTime());
    }
    ...
    public void timeWith10000PointsAnd2000Queries() {
        timeWithPointsAndQueries(10000, 2000);
    }
    ...
    public void timeWithVariousNumbersOfPoints() {
        ...
        for (int N : pointCounts) {
            timeWithPointsAndQueries(N, 10000);
        }
    }

    @Test
    public void compareTimingNaiveAndKdTree() {
        List<Point> randomPoints = randomPoints(100000);
        KDTree kd = new KDTree(randomPoints);
        NaivePointSet numPoints = new NaivePointSet(randomPoints);
        List<Point> queryPoints = randomPoints(10000);

        Stopwatch sw = new Stopwatch();
        for (Point p : queryPoints) {
            numPoints.nearest(p.getX(), p.getY());
        }
